...
export function KitchenOrderCard({ order, onUpdate }: KitchenOrderCardProps) {
    ...
    return (
        <div className="group flex flex-col p-2 sm:p-3 bg-[#1A1A2E] border-l-4 border-l-transparent hover:border-l-blue-500 rounded-r-lg transition-all duration-200 mb-2 last:mb-0 hover:bg-[#222236]">
            <div className="flex items-start justify-between gap-2 sm:gap-3">
                {/* Left: Table Info */}
                <div className="min-w-[70px] sm:min-w-[80px]">
                    <h3 className="text-white font-bold text-xs sm:text-sm leading-tight truncate">{tableName}</h3>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap gap-x-2 sm:gap-x-3 gap-y-1">
                        {order.order_items.map((item) => (
                            <div key={item.id} className="flex items-baseline gap-1 text-xs sm:text-sm">
                ...
                {/* Right: Time & Action */}
                <div className="flex items-center gap-2 sm:gap-3 min-w-[80px] sm:min-w-[100px] justify-end flex-shrink-0">
                    {/* Time */}
                    <div className={cn("flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-medium", urgencyColor)}>
                    ...
                    {order.kitchen_status === 'new' && (
                        <Button
                            size="icon"
                            className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-900/20"
                    ...
                    {order.kitchen_status === 'preparing' && (
                        <Button
                            size="icon"
                            className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-green-600 hover:bg-green-500 text-white shadow-lg shadow-green-900/20"
                    ...
                    {order.kitchen_status === 'ready' && (
                        <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-slate-800 flex items-center justify-center">
